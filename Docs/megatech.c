...
The BIOS screen is based around SMS hardware, with an additional Z80 and SMS VDP (315-5246) chip not
present on a standard Genesis.
...
static INPUT_PORTS_START( megatech ) /* Genesis Input Ports */
	...
	PORT_DIPSETTING(      0x0000, DEF_STR( On ) )
	PORT_DIPNAME( 0x0010, 0x0010, DEF_STR( Unknown ) )	// Door 1
	PORT_DIPSETTING(      0x0010, DEF_STR( Off ) )
	PORT_DIPSETTING(      0x0000, DEF_STR( On ) )
	PORT_DIPNAME( 0x0020, 0x0020, DEF_STR( Unknown ) )	// Door 2
...
static ADDRESS_MAP_START( megatech_bios_map, AS_PROGRAM, 8, mtech_state )
	...
	AM_RANGE(0x4000, 0x4fff) AM_RAM // plain ram?
	AM_RANGE(0x5000, 0x5fff) AM_RAM // battery backed ram? to 0x57FF?
	...
	AM_RANGE(0x6C00, 0x6C00) AM_WRITE(mt_coin_inhibit )
	AM_RANGE(0x7000, 0x77ff) AM_ROM // from bios rom (0x7000-0x77ff populated in ROM)
	//AM_RANGE(0x7800, 0x7fff) AM_RAM // ?
	AM_RANGE(0x8000, 0x9fff) AM_READWRITE(read_68k_banked_data, write_68k_banked_data) // window into 68k address space, reads instr rom and writes to reset banks on SMS carts?
ADDRESS_MAP_END
